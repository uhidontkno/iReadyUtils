var clicked = 0;
document.querySelector(".css-1klf3b1-ButtonWrapper.eakx3700").innerHTML = `<button class="btn btn-primary draincoins" style="font-size:12px;position:absolute;background:#2f2f6f;left:75%;top:8px;" >Drain Coins</button>`

document.querySelector(".draincoins").addEventListener("click",()=>{
    if (clicked == 0) {
        document.querySelector(".draincoins").style.background="darkred";
    document.querySelector(".draincoins").innerText = "Click again...";clicked = 1;
    } 
    else if (clicked == 1) {
  document.querySelector(".draincoins").disabled = true;    
        ...
        for (let _x00 = 0; _x00 < 128; _x00++) {fetch("https://login.i-ready.com/student-reward/student/credits/spend", {
  "headers": {
